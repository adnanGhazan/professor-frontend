  ...
  useEffect(() => {
    ...
    SocialLinkService.getPublicSocialLinks()
      ...
      .catch((err) => {
        if (process.env.NODE_ENV === "development") {
          console.error("Footer failed to load public social links:", err);
        }
      });
    ...
    SiteSettingService.getSiteSettings()
      .then((settings) => {
        ...
        if (settings.copyright_text) setCopyrightText(settings.copyright_text);
        if (settings.site_name) setSiteName(settings.site_name);
      })
      .catch((err) => {
        if (process.env.NODE_ENV === "development") {
          console.error("Footer failed to load site settings:", err);
        }
      });
      ...
      <Container size="lg" padding="normal" className="py-12 sm:py-16">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 lg:gap-12">
          ...
          <div className="md:col-span-2 space-y-4">
            <Logo title={siteName || undefined} className="text-white" />
              ...
              <div className="flex flex-wrap items-center gap-2 pt-2">
                {socialLinks.map((link) => (
                  <a
                    ...
                    className="w-9 h-9 rounded-xl bg-slate-800/80 hover:bg-slate-700 border border-slate-700/80 flex items-center justify-center text-slate-300 hover:text-amber-400 transition-all hover:scale-105 cursor-pointer"
                  >
        ...
        <div className="mt-12 pt-8 border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-500">
          <p>
            {copyrightText
              ? copyrightText
              : `© ${currentYear} ${siteName || SITE_METADATA.name || "Academic Portfolio"}. All rights reserved.`}
          </p>
